...
BOW_PATH = os.path.join(
    DATA_DIR, "ap.dat"
)  # Modify as needed (Bag-of-Words, Vowpal Wabbit format)
VOCAB_PATH = os.path.join(DATA_DIR, "vocab.txt")  # Modify as needed (Vocabulary File)
...
def compute_elbo(LAMBDA, GAMMA, PHI, train_articles, train_nonzero_idxs, C):
    ...
    E_log_p_beta = 0
    for k in range(C):
        E_log_p_beta += (ETA - 1) * np.sum(
            digamma(LAMBDA[k]) - digamma(np.sum(LAMBDA[k]))
        )
    ...
    for i in range(N):
        ...
        for idx in nonzero_idx:
            ...
            E_log_p_xz += article[idx] * np.sum(
                PHI[i][corr_idx] * (digamma(GAMMA[i]) - digamma(np.sum(GAMMA[i])))
            )

            ### E[log p(X_ij|BETA,Z_ij)]
            E_log_p_xz += article[idx] * np.sum(
                PHI[i][corr_idx]
                * (digamma(LAMBDA[:, idx]) - digamma(np.sum(LAMBDA, axis=1)))
            )
    ...
    for k in range(C):
        ...
        E_log_q_beta += -np.sum(
            (LAMBDA[k] - 1) * (digamma(LAMBDA[k]) - digamma(np.sum(LAMBDA[k])))
        )
    ...
    for i in range(N):
        ...
        E_log_q_theta += -np.sum(
            (GAMMA[i] - 1) * (digamma(GAMMA[i]) - digamma(np.sum(GAMMA[i])))
        )
    ...
    for i in range(N):
        ...
        corr_idx = 0
        for idx in nonzero_idx:
            E_log_q_z += -article[idx] * np.sum(
                PHI[i][corr_idx] * np.log(PHI[i][corr_idx])
            )
...
def run_cavi(
    ...
    for t in range(max_iter):
            ...
            for idx in nonzero_idx:
                ...
                try:
                    ...
                    raise AssertionError(
                        "phi_ij: {}, Sum: {}".format(PHI_ij, np.sum(PHI_ij))
                    )
        ...
        # Compute ELBO
        elbo = compute_elbo(
            LAMBDA_t, GAMMA_t, PHI_t, train_articles, train_nonzero_idxs, C
        )
        elbos.append(elbo)
...
def compute_pred_score(
    LAMBDA, GAMMA, PHI, test_articles, test_nonzero_idxs, test_word_idxs
):
...
def main(**kwargs):
    ...
    for trial in range(n_trials):
        ...
        # Run CAVI
        LAMBDA_final, GAMMA_final, PHI_final, elbos = run_cavi(
            LAMBDA, GAMMA, PHI, train_articles, train_nonzero_idxs, C, max_iter
        )
    ...
    if predict_flag:
        ...
        # Run CAVI
        LAMBDA_pred, GAMMA_pred, PHI_pred, elbos = run_cavi(
            LAMBDA_fixed,
            GAMMA,
            PHI,
            test_articles,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_cached", help="Option to use cached article data", action="store_true"
    )
    parser.add_argument(
        "--predict",
        help="Option to do predictive scoring on test documents",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--max_iter", help="Maximum number of iterations for CAVI", default=MAX_ITER
    )
    parser.add_argument(
        "--n_trials", help="Number of CAVI trials to execute", default=N_TRIALS
    )
